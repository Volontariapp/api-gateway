...
# Le dossier config est nécessaire au runtime pour @volontariapp/config
COPY --from=builder --chown=nestjs:nodejs /app/config ./config

USER nestjs
EXPOSE ${PORT}

# Note: Pas de healthcheck configuré car @volontariapp/health-check-nest n'est pas présent
# Si un endpoint /health est ajouté plus tard, on pourra décommenter la ligne suivante :
# HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
#     CMD node -e "fetch(`http://localhost:${PORT}/api/v1`).then(r => r.status < 500 ? process.exit(0) : process.exit(1)).catch(() => process.exit(1))"
